...
class FlomaTest(unittest.TestCase):
    ...
    # def test_pow(self):
    #     with open('floma_code/pow.py') as f:
    #        compiler.compile(f.read(),
    #             target = 'c++',
    #             output_filename = '_code/pow',
    #             output_cpp_filename='_code/pow.cpp')
        
    #     module_path = "_code/pow.so"
    #     module_name = "pow"
    ...
    #     x = m.make__dfloat(3.14, 0.0)
    #     n = 3

    #     x_to_n = m.mypow(x.val, n)
    #     print(f"{x.val} ^ {n} = {x_to_n} (should be {x.val**n})")

        # m.d_mypow(x, n, k)

        # dx = float(n)*(x.val)**(n - 1)
        # print(f"expected answer: {dx} ; actual answer: {x.dval}")
        # assert abs(dx - x.dval) < epsilon
